  ...
  for (const b of [...buchungen].sort((a, b) => a.datum.localeCompare(b.datum))) {
    ...
    if (b.typ === 'kauf') {
      ...
      if (div.istAktiendividendeAlsKauf(b)) {
        ...
        if (z > 0 && pf > 0) roh.push({ date: d, amount: z * pf })
      }
...
for (const pf of [bar / aktien, (bar / aktien) * 0.615, 0.084927, bar / (aktien + bar)]) {
  console.log('pairFrac', pf.toFixed(6), '→', irrWithPairFrac(pf)?.toFixed(4), '%')
}
