...
for (const b of buchungen) {
  if (div.istAktiendividendeAlsKauf(b)) aktienSum += xirr.irrBetragFuerKauf(b)
  if (b.typ === 'dividende' || b.typ === 'zins') barDiv += b.betragEur
}

function skipAktienFlows() {
  const flows = []
...
const flows = skipAktienFlows()

// search alpha: T_terminal = T - aktienSum - alpha * barDiv
let lo = 0
let hi = 2
for (let i = 0; i < 60; i++) {
  const mid = (lo + hi) / 2
  const term = T - aktienSum - mid * barDiv
  const v = berechneIrrAnnualizedPercent(flows, term, asOf)
  if (v < target) lo = mid
  else hi = mid
}
const alpha = (lo + hi) / 2
const term = T - aktienSum - alpha * barDiv
console.log('alpha', alpha.toFixed(4), 'term', term.toFixed(2), 'irr', berechneIrrAnnualizedPercent(flows, term, asOf))

// divAll with partial pair at T
function divAllPartialPair() {
  const roh = []
  for (const b of [...buchungen].sort((a, b) => a.datum.localeCompare(b.datum))) {
    const d = new Date(`${b.datum}T12:00:00`)
    if (b.typ === 'kauf') {
      ...
      if (div.istAktiendividendeAlsKauf(b)) {
        ...
        if (z > 0) roh.push({ date: d, amount: z * alpha }) // reuse alpha?
      }
...
console.log('divAll partial pair alpha @ T:', berechneIrrAnnualizedPercent(divAllPartialPair(), T, asOf))
