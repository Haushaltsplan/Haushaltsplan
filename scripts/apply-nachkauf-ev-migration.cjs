/**
 * Wendet EV-Multiples-Migration für nachkauf_radar_scan an (oder kopiert SQL).
 */
const fs = require('fs')
...
const sqlPath = path.join(
  ...
  'supabase',
  'migrations',
  '20260810210000_nachkauf_radar_ev_multiples.sql',
)
...
async function main() {
  ...
  if (dbUrl) {
    ...
    await client.end()
    console.log('OK — EV-Spalten Migration angewendet.')
    return
  }
  const ref = projectRefFromUrl(env.NEXT_PUBLIC_SUPABASE_URL || '')
  const dash = ref
  ...
  if (process.platform === 'win32') {
    const tmp = path.join(os.tmpdir(), `nachkauf-ev-${Date.now()}.sql`)
