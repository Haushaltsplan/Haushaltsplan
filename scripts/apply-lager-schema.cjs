/**
 * Wendet supabase/LAGER_SCHEMA_SETUP.sql an — oder kopiert SQL in die Zwischenablage.
 *
 * Automatisch: DATABASE_URL in .env.local (Supabase → Settings → Database → URI)
 *
 * Ohne DATABASE_URL: SQL → Zwischenablage + Link zum SQL-Editor.
 */
...
const sqlPath = path.join(root, 'supabase', 'LAGER_SCHEMA_SETUP.sql')
...
function copySqlToClipboard(sql) {
  if (process.platform === 'win32') {
    const tmp = path.join(os.tmpdir(), `lager-schema-${Date.now()}.sql`)
...
async function main() {
  ...
  if (dbUrl) {
    ...
    console.log('Verbinde mit Datenbank und führe LAGER_SCHEMA_SETUP.sql aus …')
    await client.connect()
    await client.query(sql)
    await client.end()
    console.log('Fertig — Tabellen und Policies sind angelegt. Seite im Browser neu laden.')
    return
  }
  ...
  console.log('\nAutomatisch beim nächsten Mal: DATABASE_URL in .env.local → npm run db:lager\n')
