...
function irrWithPairFactor(pairFrac) {
  const roh = []
  for (const b of [...buchungen].sort((a, b) => a.datum.localeCompare(b.datum))) {
    ...
    if (b.typ === 'kauf') {
      ...
      if (div.istAktiendividendeAlsKauf(b)) {
        ...
        if (z > 0) roh.push({ date: d, amount: z * pairFrac })
      }
...
for (let i = 0; i < 60; i++) {
  const mid = (lo + hi) / 2
  const v = irrWithPairFactor(mid)
  if (v < target) lo = mid
  else hi = mid
}
const pairFrac = (lo + hi) / 2
console.log('pairFrac (synthetic div / aktiendiv):', pairFrac.toFixed(6), '→', irrWithPairFactor(pairFrac), '%')
console.log('equiv neg factor:', (1 - pairFrac).toFixed(6))
