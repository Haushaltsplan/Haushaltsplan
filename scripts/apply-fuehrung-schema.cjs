/**
 * Wendet supabase/migrations/*fuehrung*.sql an.
 * DATABASE_URL / SUPABASE_DB_URL / DIRECT_URL oder SUPABASE_DB_PASSWORD in .env.local
 */
const fs = require('fs')
...
function fuehrungSqlDateien() {
  return fs
    .readdirSync(migrationsDir)
    .filter((f) => f.includes('fuehrung') && f.endsWith('.sql'))
    .sort()
...
function copySqlToClipboard(sql) {
  ...
  const tmp = path.join(os.tmpdir(), `fuehrung-schema-${Date.now()}.sql`)
...
async function main() {
  ...
  const sqlPaths = fuehrungSqlDateien()
  if (sqlPaths.length === 0) {
    console.error('Keine fuehrung-Migrationen in', migrationsDir)
    process.exit(1)
  }
  ...
  if (dbUrl) {
    ...
    console.log('Verbinde mit Datenbank und führe fuehrung-Migrationen aus …')
  ...
  console.log('\nAutomatisch: DATABASE_URL oder SUPABASE_DB_PASSWORD in .env.local → npm run db:fuehrung\n')
