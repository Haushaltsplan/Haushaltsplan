...
const flows = pairedFlows()
const Tadj = T - aktienSum
console.log('full pair @ T-aktienSum', Tadj, berechneIrrAnnualizedPercent(flows, Tadj, asOf))

// find T adj for 6.43 with full pair
let lo = 70000, hi = 95000
for (let i = 0; i < 60; i++) {
  const mid = (lo + hi) / 2
...
const tNeed = (lo + hi) / 2
console.log('T needed full pair:', tNeed, 'delta from T', T - tNeed, 'aktienSum', aktienSum)

// divAll flows with paired aktiendiv only (not skip)
function divAllPairedAktien() {
  const roh = []
  for (const b of [...buchungen].sort((a, b) => a.datum.localeCompare(b.datum))) {
    const d = new Date(`${b.datum}T12:00:00`)
    if (b.typ === 'kauf') {
...
console.log('divAll+full pair aktiendiv @ T:', berechneIrrAnnualizedPercent(divAllPairedAktien(), T, asOf))
