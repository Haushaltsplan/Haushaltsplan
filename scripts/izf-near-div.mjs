...
const divDates = new Map()
for (const b of buchungen) {
  if (b.typ !== 'dividende' && b.typ !== 'zins') continue
  const isin = b.isin?.toUpperCase() ?? ''
  if (!divDates.has(isin)) divDates.set(isin, [])
  divDates.get(isin).push(b.datum)
}
...
for (const b of buchungen) {
  ...
  const isin = b.isin?.toUpperCase() ?? ''
  const dates = divDates.get(isin) ?? []
  const hasNear = dates.some((d) => Math.abs(new Date(d) - new Date(b.datum)) < 45 * 86400000)
  if (hasNear) near++
}
console.log('aktienDiv with dividende typ within 45d:', near, '/ 34')
