...
export function BesitzKleiderschrank({ zeilen, laden, onBearbeiten, onLoeschen, formatEur, formatDatumDe }: Props) {
  ...
  if (laden) {
    return <p className="py-16 text-center text-slate-500">Kleiderschrank wird geladen …</p>
  }

  if (!kleidungZeilen.length && !schuhZeilen.length) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-700/80 bg-slate-950/40 px-6 py-16 text-center">
        <p className="text-base font-semibold text-slate-300">Noch kein Kleidung oder Schuhe</p>
  ...
  function renderKarte(z: BesitzKleiderschrankRow) {
    ...
    return (
      <article
        key={z.id}
        className="group flex flex-col overflow-hidden rounded-2xl border border-slate-800/90 bg-gradient-to-b from-slate-900/80 to-slate-950/90 shadow-sm transition hover:border-amber-800/40 hover:shadow-md hover:shadow-amber-950/20"
      >
        <div className="relative aspect-[3/4] w-full overflow-hidden bg-slate-950">
          {thumb ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-2 bg-slate-900/60 text-slate-600">
          ...
          <div className="absolute left-2 top-2">
            <span className="rounded-lg border border-slate-700/80 bg-slate-950/85 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-amber-200/95 backdrop-blur-sm">
              {artBadge(z.kleidungsart)}
            </span>
          </div>
        </div>
        <div className="flex flex-1 flex-col gap-1.5 p-3">
          <p className="line-clamp-2 text-sm font-semibold leading-snug text-slate-100">{z.name}</p>
          {z.hersteller ? <p className="truncate text-xs text-slate-400">{z.hersteller}</p> : null}
          <div className="mt-auto flex flex-wrap gap-1.5 pt-1">
            {z.groesse ? (
              <span className="rounded-md bg-slate-800/80 px-1.5 py-0.5 text-[10px] font-semibold text-slate-300">{z.groesse}</span>
            ) : null}
            {z.farbe ? (
              <span className="rounded-md bg-slate-800/80 px-1.5 py-0.5 text-[10px] font-semibold text-slate-300">{z.farbe}</span>
            ) : null}
          </div>
          <div className="flex items-end justify-between gap-2 pt-2">
            <span className="text-sm font-bold tabular-nums text-amber-200">{formatEur(Number(z.einkaufspreis_eur))}</span>
            <span className="text-[10px] tabular-nums text-slate-500">{formatDatumDe(z.einkaufsdatum)}</span>
          </div>
          <div className="mt-1 flex gap-1.5">
            <button
              type="button"
              onClick={() => onBearbeiten(z)}
              className="flex-1 rounded-lg border border-slate-600/90 py-1.5 text-[11px] font-semibold text-sky-200 transition hover:bg-sky-500/10"
            >
              Bearbeiten
            </button>
            <button
              ...
              className="rounded-lg border border-slate-600/90 px-2.5 py-1.5 text-[11px] font-semibold text-rose-300/90 transition hover:bg-rose-500/10"
            >
              ×
            </button>
          </div>
  ...
  function renderBereich(
    ...
    return (
      <div className="space-y-8">
        ...
        {gruppen.map((g) => (
          <section key={g.label}>
            <h4 className="mb-3 flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-slate-500">
              <span className="h-px flex-1 bg-slate-800/90" />
              {g.label}
              <span className="rounded-full bg-slate-800/80 px-2 py-0.5 text-[10px] tabular-nums text-slate-400">{g.items.length}</span>
              <span className="h-px flex-1 bg-slate-800/90" />
            </h4>
