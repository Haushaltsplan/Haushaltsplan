    ...
    <PageChrome>
      ...
      <section className={pageSectionShellClass}>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-stretch sm:gap-6">
          <div className="flex min-h-[200px] min-w-0 flex-1 flex-col items-center justify-center rounded-xl border border-dashed border-[var(--app-border-strong)] bg-[var(--app-surface-muted)] p-3">
            {blick.length > 0 ? (
              <div
              ...
              >
                {blick.map((b, i) => (
                  <div
                    key={b.key}
                    className="relative aspect-video overflow-hidden rounded-lg border border-[var(--app-border)] bg-[var(--app-surface-muted)]"
                  >
                    <img
                      src={b.url}
                      alt={`Bild ${i + 1}`}
                    ...
                    <button
                      ...
                      className="absolute right-1 top-1 flex h-7 w-7 items-center justify-center rounded-md border border-[var(--app-border-strong)] bg-[var(--app-surface-muted)] text-sm text-[var(--app-text)]"
                      title="Bild entfernen"
                    >
                      ×
                    </button>
                    <span className="absolute bottom-1 left-1 rounded bg-black/50 px-1.5 text-[10px] text-[var(--app-text)]">
                      {i + 1}
                    </span>
                  </div>
                ))}
              ...
              <p className="text-center text-sm text-[var(--app-text-muted)]">Noch keine Bilder</p>
            )}
          </div>
          ...
          <div className="flex w-full flex-col justify-center gap-3 sm:max-w-xs">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="rounded-xl border border-[var(--app-border-strong)] px-3 py-3 text-sm text-[var(--app-text)] hover:bg-[var(--app-surface-hover)] disabled:opacity-40"
              >
                Leeren
              </button>
            </div>
      ...
      {ergebnis && (
        <section className="space-y-4" aria-live="polite">
          <div className="flex flex-wrap items-center gap-2">
            {kInfo && (
              <span className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--app-border-strong)] bg-[var(--app-surface-muted)] px-3 py-1.5 text-sm font-semibold text-[var(--app-text)]">
            ...
            <span
              className="text-sm font-bold tabular-nums text-[var(--app-text)]"
          ...
          <h2 className="text-lg font-bold text-[var(--app-text)]">
            ...
            {ergebnis.wissenschaftlicher_name && (
              <span className="ml-2 font-mono text-sm font-normal text-[var(--app-text-muted)] italic">
                {ergebnis.wissenschaftlicher_name}
              </span>
            )}
          </h2>
          ...
          <div className="prose prose-invert max-w-none prose-p:text-[var(--app-text)] prose-p:leading-relaxed">
            <p className="whitespace-pre-wrap text-[var(--app-text)]">{ergebnis.kurztext}</p>
          </div>

          <div className="rounded-xl border border-[var(--app-border)] bg-[var(--app-surface-muted)]/50 p-4">
            <h3 className="mb-2 text-xs font-bold uppercase tracking-wide text-[var(--app-text-muted)]">Sichtbar im Bild</h3>
            <p className="whitespace-pre-wrap text-sm text-[var(--app-text)]">{ergebnis.merkmale_im_bild}</p>
          </div>
          ...
          <div className="rounded-xl border border-[var(--app-border-strong)] bg-[var(--app-surface-muted)] p-4 text-sm text-[var(--app-text-muted)]">
            <strong className="text-[var(--app-text)]">Hinweis: </strong>
