...
const TYP_CLASS = {
  ...
  sonstiges: 'bg-zinc-700/40 text-zinc-400 ring-zinc-600/40',
} as const
    ...
    <PaCard className="space-y-3 p-4">
      ...
      {daten?.nettoKaufUsd != null || daten?.kaufSummeUsd != null ? (
        <div className="flex flex-wrap gap-2 text-xs">
          ...
          {daten.nettoKaufUsd != null ? (
            <span className="rounded-full bg-zinc-800/80 px-2.5 py-0.5 text-zinc-300">
      ...
      <div className="space-y-2">
        {(daten?.transaktionen ?? []).map((t) => (
          <article key={t.id} className="rounded-xl border border-zinc-800/90 bg-zinc-950/40 px-3 py-2.5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-[10px] text-zinc-500">{t.quelle === 'sec_form4' ? 'Form 4' : 'EU IR'}</span>
              {t.datum ? <span className="text-[10px] text-zinc-500">{t.datum}</span> : null}
            </div>
            <p className="mt-1 text-sm font-medium text-white">
              {t.person}
              {t.titel ? <span className="font-normal text-zinc-500"> · {t.titel}</span> : null}
            </p>
      ...
      {daten?.hinweis && daten.transaktionen.length ? (
        <p className="text-[10px] text-zinc-600">{daten.hinweis}</p>
      ) : null}
    </PaCard>
  )
}
