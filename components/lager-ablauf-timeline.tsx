...
const BUCKETS: Bucket[] = [
  ...
  { key: 'spaeter', titel: 'In 31–90 Tagen', ton: 'border-slate-700/60 bg-slate-900/50', punkt: 'bg-slate-400', von: 31, bis: 90 },
]
...
export function LagerAblaufTimeline({ items, onOeffnen }: Props) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-800 bg-slate-900/95 shadow-md shadow-black/20">
      <button
        type="button"
      ...
      >
        <div className="min-w-0">
          <h2 className="text-sm font-bold text-slate-100 sm:text-base">Ablauf-Kalender</h2>
          <p className="text-[10px] text-slate-500 sm:text-[11px]">Was in den nächsten 90 Tagen abläuft</p>
        </div>
        <div className="flex shrink-0 items-center gap-2.5">
          <span className="rounded-md border border-slate-700 bg-slate-950/60 px-2 py-0.5 text-[11px] font-bold tabular-nums text-slate-300">{gesamt}</span>
          <CollapsibleRowHeaderEnd open={offen} labels={LABEL_EINKLAPPEN} tone="neutral" size="sm" />
        </div>
      </button>

      {offen && (
        <div className="space-y-2.5 border-t border-slate-800/80 px-3 pb-3 pt-2.5 sm:px-4">
          {gruppen.length === 0 ? (
            ...
            gruppen.map((g) => (
              <div key={g.key} className={`rounded-xl border px-3 py-2.5 ${g.ton}`}>
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="text-[12px] font-bold uppercase tracking-wide text-slate-200">{g.titel}</span>
                  <span className="text-[11px] font-semibold tabular-nums text-slate-500">{g.eintraege.length}</span>
                </div>
                <div className="space-y-1">
                  {g.eintraege.map((it) => (
                    <button
                      ...
                      className="flex w-full items-center justify-between gap-2 rounded-lg border border-slate-700/50 bg-slate-950/40 px-2.5 py-1.5 text-left transition hover:bg-slate-800/50"
                    >
                      <span className="min-w-0 truncate text-[13px] font-semibold text-slate-200">{it.name}</span>
