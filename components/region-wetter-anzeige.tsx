...
function PfeilWindAusRichtung({ grad }: { grad: number }) {
  return (
    <div
      className="relative flex h-12 w-12 items-center justify-center rounded-full border border-slate-600/60 bg-slate-950/50"
      title={`Wind aus ${windHimmelsrichtungAusGrad(grad)} (${Math.round(grad)}°)`}
    >
      <span className="absolute text-[8px] font-bold text-slate-500" style={{ top: 2 }}>
        N
      </span>
      <svg
...
export function RegionWetterAnzeige({ wetter, aktualisiertAnzeige, ortId, ortName }: Props) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-sky-800/50 bg-gradient-to-br from-sky-950/60 via-slate-900/95 to-slate-950 shadow-xl shadow-sky-950/20">
        ...
        <div>
          ...
          <div className="mt-4 flex flex-wrap items-end gap-3">
            ...
            <div className="pb-1.5">
              <p className="text-lg font-semibold text-slate-200">{wetter.zustandDe}</p>
              {wetter.feelsLikeC != null ? (
                <p className="text-sm text-slate-400">Gefühlt wie {wetter.feelsLikeC}°C</p>
              ) : null}
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-3 border-t border-slate-800/80 bg-slate-950/40 px-6 py-5 sm:grid-cols-2 sm:px-8">
        <div className="flex gap-4 rounded-xl border border-slate-800/60 bg-slate-900/50 p-4">
          <WindIkon />
          <div className="min-w-0">
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Wind</p>
            {wetter.windKmh != null ? (
              <p className="mt-1 text-lg font-bold tabular-nums text-slate-100">{wetter.windKmh} km/h</p>
            ) : (
            ...
            {grad != null ? (
              <p className="mt-1 text-sm text-slate-300">
                {windHimmelsrichtungKurz(grad)} <span className="text-slate-500">({Math.round(grad)}°)</span>
              </p>
            ) : null}
          </div>
        ...
        <div className="space-y-2 rounded-xl border border-slate-800/60 bg-slate-900/50 p-4 text-sm text-slate-300">
          {wetter.luftfeuchte != null ? (
            <p>
              <span className="text-slate-500">Luftfeuchtigkeit</span>{' '}
              <span className="font-semibold text-slate-100">{wetter.luftfeuchte}%</span>
            </p>
          ) : null}
          {wetter.tMin != null && wetter.tMax != null ? (
            <p>
              <span className="text-slate-500">Heute</span>{' '}
            ...
            <div className="flex snap-x snap-mandatory gap-2.5 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {(wetter.stundenPrognose ?? []).map((s) => {
                ...
                return (
                  <div
                    key={s.zeitIso}
                    className="flex min-w-[3.5rem] snap-center flex-col items-center gap-1 rounded-xl border border-slate-800/60 bg-slate-900/45 px-2 py-2.5"
                    title={s.zustandDe}
                  >
                    <p className="text-[10px] font-mono font-bold tabular-nums text-slate-500">{uhr}</p>
                    <WetterHimmelIcon kategorie={katS} pixel={40} className="opacity-95" />
                    <p className="text-sm font-black tabular-nums text-slate-100">{s.tempC}°</p>
                    {s.windKmh != null ? (
                      <p className="text-[8px] tabular-nums text-slate-500">{s.windKmh} km/h</p>
                    ) : null}
                  </div>
                )
              })}
