...
function ortKopfzeile(ortId: WetterOrtId) {
  if (ortId === 'leogang') {
    return (
      <p className="mb-3 text-xs text-[var(--app-text-muted)]">
        <span className="font-semibold text-[var(--app-text)]">{REGION_LEOGANG.name}</span>
        <span className="mx-1.5 text-[var(--app-text-muted)]">·</span>
  ...
  return (
    <p className="mb-3 text-xs text-[var(--app-text-muted)]">
      <span className="font-semibold text-[var(--app-text)]">{REGION_HAARBACH.name}</span>
      <span className="mx-1.5 text-[var(--app-text-muted)]">·</span>
...
export async function StartNewsUmgebungPanel() {
  ...
  return (
    <PageSectionPanel density="compact">
      <details className="app-disclosure group bg-[var(--app-surface-muted)]/20">
        <summary className="flex cursor-pointer list-none select-none items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-left outline-offset-2 transition-colors hover:bg-[var(--app-surface-hover)]/35 focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-500/50 sm:px-4">
        ...
        <div className="px-3 pb-3 pt-1 sm:px-4">
          ...
          {news.artikel.length === 0 && !news.fehler ? <p className="mt-2 text-sm text-[var(--app-text-muted)]">Keine Meldungen.</p> : null}
          <ul className="mt-3 space-y-2.5">
            {news.artikel.map((a, i) => (
              <li
                key={a.href + i}
                className="flex flex-col gap-0.5 border-b border-[var(--app-border)] pb-2.5 last:border-0 last:pb-0 sm:flex-row sm:items-baseline sm:gap-3"
              >
                <time
                  className="shrink-0 text-xs font-mono tabular-nums text-[var(--app-text-muted)]"
                ...
                <div className="min-w-0 flex-1">
                  <a
                    ...
                    className="text-[15px] font-semibold text-[var(--app-text)] underline decoration-[var(--app-border-strong)] underline-offset-2 transition hover:text-cyan-200"
                  >
                    {a.titel}
                  </a>
                  <span className="ml-1.5 text-xs text-[var(--app-text-muted)]">· {a.quelle}</span>
                </div>
              </li>
            ))}
          </ul>
      ...
      <details className="app-disclosure group bg-[var(--app-surface-muted)]/20">
        ...
        <div className="px-3 pb-3 pt-1 sm:px-4">
          ...
          {portfolioNews.artikel.length === 0 && !portfolioNews.fehler ? (
            <p className="mt-2 text-sm text-[var(--app-text-muted)]">Keine Meldungen.</p>
          ) : null}
          <ul className="mt-3 space-y-2.5">
