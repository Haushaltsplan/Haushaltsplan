...
export function TerminMorgenReminderEinstellungen() {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-muted)] shadow-md shadow-black/20">
      <div className="border-b border-[var(--app-border)] px-4 py-3 sm:px-5">
        <h2 className="text-sm font-black text-[var(--app-text)] sm:text-base">Tägliche Kalender-Erinnerung</h2>
        <p className="mt-1 text-[11px] leading-relaxed text-[var(--app-text-muted)] sm:text-xs">
          Wenn an einem Tag <strong className="text-[var(--app-text)]/90">irgendein</strong> Kalendereintrag hinterlegt ist (Termin,
      ...
      <div className="space-y-3 px-4 py-4 sm:px-5">
        ...
        {perm === 'unsupported' ? (
          <p className="text-xs text-[var(--app-text-muted)]">Dieser Browser unterstützt keine System-Benachrichtigungen.</p>
        ) : (
          <>
            <label className="flex cursor-pointer items-start gap-3">
              <input
                type="checkbox"
                className="mt-1 h-4 w-4 rounded border-[var(--app-border-strong)] bg-[var(--app-surface-muted)] text-teal-600 focus:ring-teal-500/40"
              ...
              />
              <span>
                <span className="block text-sm font-bold text-[var(--app-text)]">Benachrichtigung nutzen</span>
            ...
            <div className="flex flex-wrap items-end gap-3">
              <label className="block text-[10px] font-black uppercase tracking-wider text-[var(--app-text-muted)]">
                Uhr (lokal)
                <input
                  type="number"
                  min={0}
                  max={23}
                  className="mt-1 w-20 rounded-lg border border-[var(--app-border-strong)] bg-[var(--app-surface-muted)] py-2 px-2.5 text-sm font-bold text-[var(--app-text)] outline-none focus:ring-2 focus:ring-teal-500/45"
                  value={einst.stunde}
              ...
              <button
                type="button"
                className="rounded-lg border border-[var(--app-border-strong)] bg-[var(--app-surface-hover)] px-3 py-2 text-xs font-bold text-[var(--app-text)] transition hover:bg-[var(--app-surface-hover)] disabled:opacity-40"
            ...
            <p className="text-[10px] leading-relaxed text-[var(--app-text-muted)]">
              In der Meldung erscheint pro Eintrag <strong className="text-[var(--app-text-muted)]">Kategorielabel: Titel</strong>, z. B. „Geburtstag: …“
              oder „Urlaub: …“.
            </p>
          </>
        )}
